...
/*func getOperator(s string) string {
	...
	if operator == "" {
		panic("Операция не задана")
	}

	if strings.Count(s, operator) != 1 {
		panic("Количество операций должно быть равно 1: " + operator)
	}
...
func main() {
	...
	for {
		...
		// убираем лишние символы пробела и переноса строки
